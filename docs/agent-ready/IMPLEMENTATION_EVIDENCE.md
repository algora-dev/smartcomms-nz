# SmartComms NZ â€” Release 1 Implementation Evidence

**Implemented:** 2026-09-20 Â· Commits Aâ€“D on `main` (local; push pending owner review)
...
| D `SC-03/SC-06: honest content dates...` | content-meta: strict helpers (`reviewedDateStrict`/`publishedDateStrict` throw on missing entries), legacy helpers no longer fabricate `2026-09-11` (warn in dev, empty â†’ omitted in sitemap), `/systems/ip-intercom` datePublished now uses the published source, school-compare JSON-LD `<`â†’`\u003c` real escape, funding (`smartcomms-nz-funding-v2`) / finance (`smartcomms-nz-finance-v1`) version ids, robots: GPTBot training opt-out + `/insights/`,`/api/` excluded in every agent group. |

## Gate results (exact commands)

- `npx --no-install tsc --noEmit` â†’ **exit 0**
- `npm run lint` â†’ **exit 0** (0 errors, 0 warnings after fix)
- `npm run test:pricing` â†’ all assertions passed (A: $10,276â€“$12,845; safety: $11,792; interactive: $14,072; endpoints 34 â€” unchanged)
- `npm run test:validation` â†’ all 14 assertions passed (null-speaker semantics, false/0 preservation, unknown-field rejection incl. injected `email`/`note`, enum/count/station/two-way bounds, older-cfg defaults, interactive fixture $14,072â€“$17,590)
- `npm run test:finance` â†’ all assertions passed
- `npm run test:aged-care` â†’ passed (NZ$11,892â€“$14,865, 13 endpoints â€” unchanged)
- `npm run test:public-project-examples` â†’ all 17 checks passed
- `npm run build` â†’ see build result below

## Fixture-table status

Numerical fixtures (10-room B Essential / C-unsure / Safety / Interactive / aged-care example / presets): all **unchanged**, verified via test:pricing + test:validation + test:aged-care. Forged low/high/breakdown: can no longer alter stored records â€” server recomputes from validated state; legacy client totals only ever set a boolean `legacy_client_mismatch` marker. Malformed cfg â†’ fresh calculator start + brief notice, no completion log (validation + no-scope rejection). Logger/storage failure â†’ visitor calculator unaffected (fire-and-forget 204 preserved; kill-switch added).
...
## Deployment gaps (owner action required â€” blocks sign-off, not this code)
...
3. **Clarity project settings:** verify masking/consent configuration and replay with synthetic data (code-side masking points added; dashboard verification outstanding). Note: Clarity now loads on production hosts only â€” confirm dev/preview QA override path if needed.
4. **GA4 automatic page/referrer capture** for publicâ†’private client-side navigation: runtime verification outstanding (initial-path script gating + track() guard added in code).
5. **Screenshots at 390/1440px:** visual baseline capture requires a running browser session against the built site; not performed in this pass â€” pages are unchanged by design and all numerical/UI state fixtures are frozen in tests.
