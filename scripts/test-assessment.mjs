// Release 2 assessment contract tests.
...
ok("valid 10-room B Essential returns the exact public fixture band", () => {
  const r = assessPricing(tenRoomsB);
...
ok("forged low/high in the payload cannot change the assessment result", () => {
  const forged = { ...JSON.parse(JSON.stringify(tenRoomsB)), low: 1, high: 2, breakdown: [{ label: "lie", amount: 1 }] };
...
ok("unspecified project returns needs_input, not a controller-only quote", () => {
  const r = assessPricing(defaultState());
  assert.equal(r.status, "needs_input");
  assert.ok(Array.isArray(r.missing) && r.missing.includes("areas"));
});

ok("invalid input returns needs_input with the validation reason", () => {
  const r = assessPricing({ tier: "Z", areas: { standardIndoor: -5 }, featurePackage: "premium" });
  assert.equal(r.status, "needs_input");
});
...
ok("monitoring is reported separately from installed low/high", () => {
  ...
  const r = assessPricing(withMonitoring);
  assert.equal(r.status, "ok");
...
ok("next actions link to the tool with the same cfg format and the human enquiry journey", () => {
  const r = assessPricing(tenRoomsB);
  const tool = r.next_actions.find((a) => a.url.startsWith("/pricing-tool?cfg="));
  ...
  const again = assessPricing(cfg);
  assert.equal(again.low, 10276);
  assert.ok(r.next_actions.some((a) => a.url === "/contact"));
});

ok("applied defaults are reported (older-cfg omitted fields)", () => {
  const r = assessPricing({ tier: "B", areas: { standardIndoor: 6 }, featurePackage: "essential" });
  assert.equal(r.status, "ok");
